...
class ConvInpEcho2(BaseModel):
	...
	def __init__(self,inp_dim,
		...
		self.n_echo1 = self.n_inp
		self.n_echo2 = n_echo
		self.n_echo = self.n_echo1 + self.n_echo2
		self.n_dm = n_dm
		self.out_scale_echo2 = out_scale_echo2
		...
		self.simple_echo1 = SimpleEcho(inp_num=self.n_inp,
									  hid_num=self.n_echo1,
									  **echo_dict1)
		self.simple_echo2 = SimpleEcho(inp_num=self.n_echo1,
									  hid_num=self.n_echo2,
									  **echo_dict2)
		if dm_dict is None:
	...
	def init_weights(self):
		...
		matrix = np.exp(-(X**2+Y**2)/2.0/self.input_sigma**2)*self.input_prob_scale
		...
		for xi in range(self.inp_dim[0]):
			for yi in range(self.inp_dim[1]):
				prob = matrix[self.inp_dim[0]-xi:2*self.inp_dim[0]-xi,self.inp_dim[1]-yi:2*self.inp_dim[1]-yi].reshape((1,)+self.inp_dim)
				self.conv_inp.weight.data[xi*self.inp_dim[1]+yi]= \
	  torch.FloatTensor((np.random.rand(1,self.inp_dim[0],self.inp_dim[1])<prob)*np.random.uniform(-self.input_scale1,self.input_scale1,size=(1,)+self.inp_dim))
		...
		plt.colorbar()
		plt.savefig("matrix_weight_{}_{}_{}.png".format(self.input_scale1,self.input_sigma,self.input_prob_scale))
	...
	def forward_train(self,x):
		...
		if self.with_dm:
			for i in range(T):
				...
				self.h_echo1, (self.u_echo1,_) = self.simple_echo1(input_echo.reshape(-1,self.n_inp),(self.u_echo1,self.h_echo1))
				self.h_echo2, (self.u_echo2,_) = self.simple_echo2(self.h_echo1,(self.u_echo2,self.h_echo2))
				h_echo = torch.cat([self.h_echo1,self.h_echo2*self.out_scale_echo2],dim=1)
